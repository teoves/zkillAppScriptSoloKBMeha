...
/**
 * Generates an array of year/month combinations from current date back to character's birthday
 * @param {string} characterId - Character ID to fetch birthday for
 * @return {Array} Array of objects with {year, month} properties
 */
function generateYearMonthCombinations(characterId) {
  const combinations = [];
  const now = new Date();
  const currentYear = now.getFullYear();
  const currentMonth = now.getMonth() + 1; // getMonth() returns 0-11
  
  // Get character birthday from ESI API
  const birthdayStr = getCharacterBirthday(characterId);
  let birthYear = 2003;
  let birthMonth = 5; // Default to May 2003 if birthday can't be fetched
  
  if (birthdayStr) {
    try {
      const birthDate = new Date(birthdayStr);
      birthYear = birthDate.getFullYear();
      birthMonth = birthDate.getMonth() + 1; // getMonth() returns 0-11
      Logger.log(`Using character birthday: ${birthYear}-${birthMonth.toString().padStart(2, '0')}`);
    } catch (e) {
      Logger.log(`Error parsing character birthday '${birthdayStr}': ${e.toString()}, defaulting to May 2003`);
    }
  } else {
    Logger.log(`Could not fetch character birthday, defaulting to May 2003`);
  }
  
  // Start from current year/month and go back to character's birthday
  for (let year = currentYear; year >= birthYear; year--) {
    const startMonth = (year === currentYear) ? currentMonth : 12;
    const endMonth = (year === birthYear) ? birthMonth : 1;
...
/**
 * Fetches a single page from zkillboard API with year and month filters
 ...
 */
function fetchZKillboardPage(characterID, pageNum, year, month) {
  const API_URL = `https://zkillboard.com/api/kills/characterID/${characterID}/solo/page/${pageNum}/year/${year}/month/${month}/`;
  ...
  try {
    ...
    if (response.getResponseCode() !== 200) {
      Logger.log(`Error fetching page ${pageNum} ${year}-${month.toString().padStart(2, '0')}: HTTP ${response.getResponseCode()}`);
    ...
    Logger.log(`Error processing page ${pageNum} ${year}-${month.toString().padStart(2, '0')}: ${e.toString()}`);
...
/**
 * Fetches character birthday from EVE ESI API
 * @param {string} characterId - The EVE character ID
 * @return {string|null} Character birthday as ISO 8601 date string, or null if error
 */
function getCharacterBirthday(characterId) {
  try {
    ...
    if (response.getResponseCode() !== 200) {
      Logger.log(`Error fetching character ${characterId}: HTTP ${response.getResponseCode()}`);
      return null;
    }
    
    const characterData = JSON.parse(response.getContentText());
    return characterData.birthday || null;
  } catch (e) {
    Logger.log(`Error fetching character birthday for ${characterId}: ${e.toString()}`);
    return null;
  }
}
 ...
 * @param {Object} referenceMaps - Reference data maps from loadReferenceDataMaps()
 * @return {Array} Array of complete row arrays (zkill data + ESI details + reference data)
 */
function fetchEsiDetailsForZkillData(zkillData, referenceMaps) {
